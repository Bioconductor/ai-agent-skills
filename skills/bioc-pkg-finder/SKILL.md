...
description: >
  Find the R / Bioconductor packages best suited to a task or workflow. Use when
  the user (or another agent) asks "what package should I use for X", "is there a
  Bioconductor package that does Y", "which package is standard for Z", or is
  assembling an analysis workflow and needs to pick tools. Points at the live,
  machine-readable indexes (biocViews, CRAN Task Views) rather than a hand-copied
  list, so answers stay current across Bioconductor releases.
---
