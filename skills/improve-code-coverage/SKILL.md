...
Analyzes an R package's code coverage using the `covr` package, audits existing tests for tautological or vacuous assertions (pseudo-coverage), identifies uncovered lines (specifically in `R/` and `src/`), and actively suggests and writes new test cases. It emphasizes a Bioconductor-standard approach to testing by classifying coverage and new tests into: a) normal use, b) edge cases, c) error handling, and d) correctness of results.
...
Before assuming that covered lines are reliably tested, audit existing tests (in `tests/testthat/` or `inst/unitTests/`) for anti-patterns that produce "pseudo-coverage" (high line coverage without verifying actual functionality):
...
- **Vacuous assertions**: Assertions that cannot meaningfully fail or only verify that code ran without checking output correctness (e.g., sole assertions like `expect_no_error()`, `expect_true(!is.null(x))`, or `expect_s4_class(x, ...)` without inspecting slots, dimensions, or data values).
...
- Flag any identified pseudo-coverage tests and recommend refactoring them with independent ground-truth checks.
...
- Draft new `testthat` code blocks targeting the identified gaps and refactoring any tautological tests.
...
- **Enforce anti-tautology rules in new tests**:
  ...
  - Test the actual contents, dimensions, and S4 object validities rather than just object existence or non-error status.
...
2. Audits existing tests and notes that while `R/normalize.R` has 90% line coverage, its tests use implementation-mirroring to generate expected values and only assert `expect_true(!is.null(result))`.
3. Identifies genuine missing edge case tests for `NA` inputs and 0-length count vectors.
4. Drafts new and refactored `test_that` blocks addressing those gaps with independent ground-truth assertions, labeled clearly by their category.
...
- Beware of "coverage chasing": writing tautological or vacuous assertions solely to hit the Bioconductor 80% coverage recommendation creates technical debt and false confidence. Always prioritize semantic correctness over raw line counts.
